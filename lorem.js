...
const result = document.querySelector(".paragraphs");
...
btn.addEventListener("click", function (e){
    ...
    const randomNumber = Math.floor(Math.random() * loremParagraphs.length);

    if(isNaN(value) || value <= 0 || value > 9) {

        result.innerHTML = `<p>${loremParagraphs[randomNumber]}</p>`;
    }
    else {

        let temparr = loremParagraphs.slice(0, value);
        temparr = temparr.map(function(item){

            return `<p>${item}</p>`
        }).join("");

        result.innerHTML = temparr;
    }
})